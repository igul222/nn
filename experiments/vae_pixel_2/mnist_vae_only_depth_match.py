...
def Decoder(latents):
    ...
    output = T.nnet.relu(lib.ops.conv2d.Conv2D(    'Dec.8', input_dim=DIM_1, output_dim=2*DIM_1, filter_size=3, inputs=output))
    ...
    for i in xrange(PIXEL_CNN_LAYERS):
        ...
        output = PixCNNGate(lib.ops.conv2d.Conv2D('Dec.Pix'+str(i), input_dim=inp_dim, output_dim=2*DIM_1, filter_size=PIXEL_CNN_FILTER_SIZE, inputs=output))
