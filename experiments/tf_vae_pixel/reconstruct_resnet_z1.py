...
with tf.Session(config=tf.ConfigProto(allow_soft_placement=True)) as session:
    ...
    for i in xrange(NUM_EXAMPLES):
        dev_image = dev_images[i]
        canvas[i*64:(i+1)*64, 0:64, :] = dev_image.transpose((1, 2, 0))/255.
        plt.imshow(dev_image.transpose((1, 2, 0))/255.)
        plt.savefig('{}.png'.format(i))
        for j in xrange(NUM_RECONSTS):
            dev_image = dev_images[i][None, ...]
            reconstructed_image = np.argmax(reconstruct(dev_image), axis = -1)[0]
            reconstructed_image = reconstructed_image.transpose((1, 2, 0))
            plt.imshow(reconstructed_image/255.)
            plt.savefig('{}_{}.png'.format(i, j))
            canvas[i*64:(i+1)*64, (j+1)*64:(j+2)*64, :] = reconstructed_image/255.
    
    plt.imshow(canvas)
    plt.savefig('reconstruct_{}'.format(DATASET))
