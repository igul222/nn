...
with tf.Session(config=tf.ConfigProto(allow_soft_placement=True)) as session:
    ...
    for device_index, (device, images, labels) in enumerate(zip(DEVICES, split_images, split_labels)):
        with tf.device(device):
            ...
            if MODE == 'one_level':
                ...
                if FEATURES == 'mu1':
                    classifier_output = tf.stop_gradient(mu1)
                elif FEATURES == 'latents1':
                    classifier_output = tf.stop_gradient(latents1)                    
                classifier_output = lib.ops.linear.Linear('Classifier', input_dim=LATENT_DIM_2, output_dim=10, inputs=classifier_output)
                # classifier_output = lib.ops.linear.Linear('Classifier1', input_dim=LATENT_DIM_2, output_dim=100, inputs=classifier_output)
                # classifier_output = nonlinearity(classifier_output)
                # classifier_output = tf.nn.dropout(classifier_output, keep_prob=1.-(0.5*tf.cast(bn_is_training, 'float32')))
                # classifier_output = lib.ops.linear.Linear('Classifier2', input_dim=100, output_dim=10, inputs=classifier_output)
                ...
                classifier_acc = tf.reduce_mean(
                    tf.cast(
                        tf.equal(
                            tf.to_int32(tf.argmax(classifier_output, dimension=1)),
                            labels
                        ),
                        tf.float32
                    )
                ...
                if VANILLA:
                    ...
                    nll = reconst_cost + (alpha * kl_cost_1)
                    cost = nll + classifier_cost
    ...
    if MODE == 'one_level':
        ...
        def generate_and_save_samples(tag):
            ...
            def train_svm(name, n_train_examples, multiply, clf, Encoder, train_data, evaluate_data , dimension = 16, **kwargs):
                Y_train = []
                Z_train = np.zeros((n_train_examples * multiply, dimension), dtype = 'float32')
                i = 0
                broken = False
                for (images, targets) in train_data():
                    for j in xrange(multiply):
                        new_examples = len(targets)
                        Y_train += list(targets.astype(np.int32))
                        Z_train[i: i + new_examples] = Encoder(images)

                        i += new_examples

                        if i >= n_train_examples * multiply:
                            broken = True
                            break
                    if broken:
                        break
                ...
                # assert (i == 50000)
                # if i != 50000:
                #     print "warning, i isn't 50K; it's {}".format(i)

                # print "Training sklearn model....."
                # clf = LinearSVC(max_iter = 700)
                # clf = sklearn.neighbors.KNeighborsClassifier(5, weights='uniform')
                ...
                for (images, targets) in evaluate_data():
                    ...
                    correct_classification += np.sum((targets == predictions))

                evaluation_accuracy = correct_classification/10000.0

                # print "Evaluation accuracy : {}".format(evaluation_accuracy)

                # return (train_acc, evaluation_accuracy)
                print "sklearn {} accuracy: {} train\t{} test".format(name, train_acc, evaluation_accuracy)

            def svm_encoder(_images):
                if FEATURES == 'mu1':
                    return session.run(mu1, feed_dict={images: _images, total_iters: 99999, bn_is_training: False, bn_stats_iter:0})
                elif FEATURES == 'latents1':
                    return session.run(latents1, feed_dict={images: _images, total_iters: 99999, bn_is_training: False, bn_stats_iter:0})

            # train_svm('logreg', 50000, 1, LogisticRegression(), svm_encoder, train_data, dev_data, dimension=LATENT_DIM_2)
            # train_svm('svm', 50000, 1, SVC(max_iter=700), svm_encoder, train_data, dev_data, dimension=LATENT_DIM_2)
            for k in xrange(1,10):
                train_svm('knn-{}'.format(k), 50000, 1, sklearn.neighbors.KNeighborsClassifier(k, weights='distance'), svm_encoder, train_data, dev_data, dimension=LATENT_DIM_2)
            ...
            #     train_acc, test_acc = train_svm('knn', 100, 1, sklearn.neighbors.KNeighborsClassifier(k+1, weights=weights), svm_encoder, train_data, dev_data, dimension=LATENT_DIM_2)
            #     mean_train_acc += train_acc
            #     mean_test_acc += test_acc
            # print "k={}\tweights={}\ttrain={}\ttest={}".format(k+1, weights, mean_train_acc/20., mean_test_acc/20.)

            # train_svm('knn-100-1', 100, 1, sklearn.neighbors.KNeighborsClassifier(2, weights='distance'), svm_encoder, train_data, dev_data, dimension=LATENT_DIM_2)
            # train_svm('knn-100-2', 100, 1, sklearn.neighbors.KNeighborsClassifier(2, weights='distance'), svm_encoder, train_data, dev_data, dimension=LATENT_DIM_2)
            # train_svm('knn-100-3', 100, 1, sklearn.neighbors.KNeighborsClassifier(2, weights='distance'), svm_encoder, train_data, dev_data, dimension=LATENT_DIM_2)
            # train_svm('knn-100-4', 100, 1, sklearn.neighbors.KNeighborsClassifier(2, weights='distance'), svm_encoder, train_data, dev_data, dimension=LATENT_DIM_2)
            # train_svm('knn-100-5', 100, 1, sklearn.neighbors.KNeighborsClassifier(2, weights='distance'), svm_encoder, train_data, dev_data, dimension=LATENT_DIM_2)

            # train_svm('svm-100-1', 100, 10, SVC(max_iter=700), svm_encoder, train_data, dev_data, dimension=LATENT_DIM_2)
            # train_svm('svm-100-2', 100, 10, SVC(max_iter=700), svm_encoder, train_data, dev_data, dimension=LATENT_DIM_2)
            # train_svm('svm-100-3', 100, 10, SVC(max_iter=700), svm_encoder, train_data, dev_data, dimension=LATENT_DIM_2)

            plt.clf()
            for (__images, __targets) in test_data():
                points = svm_encoder(__images)
                plt.scatter(points[:,0],points[:,-1],s=3,c=__targets, edgecolors='face')
            plt.axis([-4, 4, -4, 4])
            plt.savefig('plot_'+tag)
            print "plot_{} saved.".format(tag)
    ...
    if MODE == 'one_level':
        prints=[
            ...
            ('kl1', kl_cost_1),
            ('c_cost', classifier_cost),
            ('c_acc', classifier_acc)
        ]
    elif MODE == 'two_level':
        prints=[
            ('alpha1', alpha1),
