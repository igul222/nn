...
with tf.Session(config=tf.ConfigProto(allow_soft_placement=True)) as session:
    ...
    dev_images = dev_data_gen.next()[0][:NUM_EXAMPLES]
    ...
    def get_mu2(_images):
        return session.run([mu2], feed_dict = {all_images: _images})

    HOLD_Z2_CONSTANT = False
    HOLD_EPSILON_1_CONSTANT = False
    HOLD_EPSILON_PIXELS_CONSTANT = False

    canvas = np.zeros((NUM_EXAMPLES*64, 64*(NUM_RECONSTS+1), 3))
    for example_ind in xrange(NUM_EXAMPLES):
        dev_image = dev_images[example_ind]
        z2 = get_mu2(dev_image[None, :, :, :])[0]
        z2 = np.repeat(z2, repeats = NUM_RECONSTS, axis = 0)

        # Draw epsilon_1 from N(0,I)
        epsilon_1 = np.random.normal(size=(NUM_RECONSTS, LATENT_DIM_1, LATENTS1_HEIGHT, LATENTS1_WIDTH)).astype('float32')
        if HOLD_EPSILON_1_CONSTANT:
          epsilon_1[:] = epsilon_1[0][None]

        # Draw epsilon_pixels from U[0,1]
        epsilon_pixels = np.random.uniform(size=(NUM_RECONSTS, N_CHANNELS, HEIGHT, WIDTH))
        if HOLD_EPSILON_PIXELS_CONSTANT:
          epsilon_pixels[:] = epsilon_pixels[0][None]

        # Draw z1 autoregressively using z2 and epsilon1
        print "Generating z1"
        z1 = np.zeros((NUM_RECONSTS, LATENT_DIM_1, LATENTS1_HEIGHT, LATENTS1_WIDTH), dtype='float32')
        ...
        pixels = np.zeros((NUM_RECONSTS, N_CHANNELS, HEIGHT, WIDTH)).astype('int32')
        ...
        canvas[example_ind*64:(example_ind+1)*64, 0:64, :] = dev_image.transpose((1, 2, 0))/255.
        plt.imshow(dev_image.transpose((1, 2, 0))/255.)
        for reconst_ind in xrange(NUM_RECONSTS):
            reconstructed_image = pixels[reconst_ind, ...]
            reconstructed_image = reconstructed_image.transpose((1, 2, 0))
            plt.imshow(reconstructed_image/255.)
            plt.savefig('{}_{}.png'.format(example_ind, reconst_ind))
            canvas[example_ind*64:(example_ind+1)*64, (reconst_ind+1)*64:(reconst_ind+2)*64, :] = reconstructed_image/255.
    
    plt.imshow(canvas)
    plt.savefig('reconstruct_mu2_{}'.format(DATASET))
