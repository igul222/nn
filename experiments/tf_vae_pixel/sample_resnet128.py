...
with tf.Session(config=tf.ConfigProto(allow_soft_placement=True)) as session:
    ...
    all_latents1 = tf.placeholder(tf.float32, shape=[None, DIM_32_LATENT, LATENTS1_HEIGHT, LATENTS1_WIDTH], name='all_latents1')
    ...
    def Dec1_pixel(upsampled_z1, images):
        ...
        return tf.transpose(
            tf.reshape(output, tf.pack([-1, 256, N_CHANNELS, tf.shape(images)[2], tf.shape(images)[3]])),
            [0,2,3,4,1]
        )
    ...
    tf.train.Saver().restore(session, '/Tmp/ahmedfar/IMGNET/params.ckpt')
   ...
   # Draw z1 autoregressively using z2 and epsilon1
    ...
    RECEPTIVE_FIELD_z1 = 4   ## to be computed
