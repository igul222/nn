...
def Encoder(name, input_dim, hidden_dim, latent_dim, downsample, inputs):
    inputs = T.clip(inputs, lib.floatX(-50), lib.floatX(50))
...
def E2(inputs):
    return Encoder('E2', LATENT_DIM, L2_CONV_DIM, LATENT_DIM, True, inputs)
...
gs_latents1 = scale_grads(latents1)

mu_and_logsig2 = E2(gs_latents1)
...
def E3(inputs):
    ...
    return Encoder('E3', LATENT_DIM, L3_CONV_DIM, LATENT_DIM, True, inputs)
...
gs_latents2 = scale_grads(latents2)

mu_and_logsig3 = E3(gs_latents2)
...
def E4(inputs):
    return Encoder('E4', LATENT_DIM, L4_CONV_DIM, LATENT_DIM, True, inputs)

def D4(latents):
    return Decoder('D4', LATENT_DIM, L4_CONV_DIM, 2*LATENT_DIM, True, latents)

gs_mu_and_logsig3 = scale_grads(mu_and_logsig3)
gs_mu3, gs_logsig3 = split(gs_mu_and_logsig3)
gs_latents3 = scale_grads(latents3)

mu_and_logsig4 = E4(gs_latents3)
...
def E5(inputs):
    inputs = inputs.reshape((inputs.shape[0], 4*4*LATENT_DIM))
...
gs_mu_and_logsig4 = scale_grads(mu_and_logsig4)
gs_mu4, gs_logsig4 = split(gs_mu_and_logsig4)
gs_latents4 = scale_grads(latents4)

mu_and_logsig5 = E5(gs_latents4)
...
gs_mu5, gs_logsig5 = split(scale_grads(mu_and_logsig5))
