...
# l2_out_log_sigma = T.nnet.relu(l2_out_log_sigma, alpha=alpha)
...
if VANILLA:
    ...
    # cost = reconst_cost + ((alpha2*alpha2) * l2_reg_cost) + (alpha * l2_reconst_cost)

### Sampling code

N_SAMPLES = 1250 # 5 seconds

# z2 = theano_srng.normal((N_SAMPLES, L2_LATENT))
...
# outs = L1Decoder(z1).reshape((N_SAMPLES, FRAME_SIZE**2, Q_LEVELS))
...
lib.train_loop.train_loop(
    ...
    prints=[
        ('alpha', alpha),
        # ('nll', reconst_cost + l2_reg_cost + l2_reconst_cost),
        ('reg', reg_cost),
        # ('l2reg', l2_reg_cost),
